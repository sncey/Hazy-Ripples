...
//     expect(OrganizationModel.findOne).toHaveBeenCalledWith({ email: 'test@example.com' });
...
//     OrganizationModel.findOne.mockResolvedValue({ name: 'Existing Organization' });
...
//     OrganizationModel.findOne.mockRejectedValue(new Error(errorMessage));
...
//       // Expectations
//       expect(res.status).toHaveBeenCalledWith(500);
//       expect(res.json).toHaveBeenCalledWith({ error: 'Error in server', error: errorMessage });
//     });
//   });
...
//   it('should create a new organization account', async () => {
//     const now = Date.now();
//     const jwtExp = Math.floor(now / 1000) + 86400;
//     const token = 'test_token';
//     const emailText = 'Welcome to Test Organization!';
//     const organizationMock = {
//       save: jest.fn().mockResolvedValue(),
...
//     // Expectations
//     expect(res.status).toHaveBeenCalledWith(500);
//     expect(res.json).toHaveBeenCalledWith({ error: any(String)});
//   });
// });
...
describe('createAccount', () => {
    ...
    it('should handle other errors', async () => {
      OrganizationModel.findOne.mockRejectedValue(new Error('Error in server'));
  
      await organizationController.createAccount(req, res);
  
      // Expectations
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ error: 'Error in server',});
    });
  });